...
package body Vulkan.Performance_Queries is
    ...
    type vkEnumeratePhysicalDeviceQueueFamilyPerformanceQueryCountersKHR_Access
        is access function
            (Physical_Device: in Vulkan.Physical_Device;
             ...
             Counters: access Performance_Queries_C.Performance_Counter_C;
             Counter_Description:
                access Performance_Queries_C.Performance_Counter_Description_C)
        return Result
        with Convention => C;
        ...
        access procedure
            (Physical_Device: in Vulkan.Physical_Device;
             Peformance_Query_Create_Info:
                in Performance_Queries_C.Query_Pool_Performance_Create_Info_C;
    ...
    type vkAcquireProfilingLockKHR_Access is
        access function
            (Device: in Vulkan.Device;
             Info: in Performance_Queries_C.Acquire_Profiling_Lock_Info_C)
        return Result
        with Convention => C;
    ...
    function Get_Performance_Query_Counters
        ...
        Counters_C: array (1 .. Positive(Counters.Length)) of aliased
            Performance_Queries_C.Performance_Counter_C;
        Counter_Descriptions_C: array (Counters_C'Range) of aliased
            Performance_Queries_C.Performance_Counter_Description_C;
    ...
    begin
        ...
        for X in Counters_C'Range loop
            Performance_Queries_C.To_Ada(Counters(X), Counters_C(X));
            Performance_Queries_C.To_Ada(Counter_Descriptions(X),
    ...
    procedure Get_Performance_Query_Counters
        (Physical_Device: in Vulkan.Physical_Device;
         ...
         Counters: in out Performance_Counter_Vectors.Vector;
         Counter_Descriptions:
            in out Performance_Counter_Description_Vectors.Vector) is
    begin
    ...
    function Get_Performance_Query_Passes
        ...
        Performance_Query_Create_Info_C:
            Performance_Queries_C.Query_Pool_Performance_Create_Info_C :=
                Performance_Queries_C.To_C(Performance_Query_Create_Info);
        Count: Interfaces.Unsigned_32;
    begin
        vkGetPhysicalDeviceQueueFamilyPerformanceQueryPassesKHR
            (Physical_Device,
             Performance_Query_Create_Info_C,
             Count);
        Performance_Queries_C.Free(Performance_Query_Create_Info_C);
    ...
    function Acquire_Profiling_Lock(Device: in Vulkan.Device;
        ...
        return Result is
        Info_C: Performance_Queries_C.Acquire_Profiling_Lock_Info_C :=
            Performance_Queries_C.To_C(Info);
        Result: Vulkan.Result;
    begin
        Result := vkAcquireProfilingLockKHR(Device, Info_C);
        Performance_Queries_C.Free(Info_C);
