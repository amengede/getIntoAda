...
private
    ...
    -- @param Point line origin
    -- @param Slope line slope
    procedure Draw_Bresenham_Steep (Point, Slope : in Point_2D);

    procedure Subdivide (Tile : Rasterization_Tile);

    procedure Rasterize_With_Tests (Tile : Rasterization_Tile);

    procedure Fill_Rectangle (X_Min, Y_Min, X_Max, Y_Max : Integer);

    type Command_Type is (Clear_Row,
                         Fill_Rectangle,
                         Rasterize_Tile_With_Tests);
    type Command_Int_Buffer is array (0 .. 19) of Integer;
    type Draw_Command is
        record
            Id : Command_Type;
            Int_Args : Command_Int_Buffer;
        end record;

    function Pack_Tile_Into_Buffer (Tile : in Rasterization_Tile)
        return Command_Int_Buffer;
    function Unpack_Tile_From_Buffer (Buffer : in Command_Int_Buffer)
        return Rasterization_Tile;

    package Work_Queues is new Queues (T => Draw_Command);
